...
@router.post("/{parcel_id}/deliver", summary="Marquer livré (+ preuve)")
async def deliver_parcel(
    ...
    updated = await transition_status(
        parcel_id, ParcelStatus.DELIVERED,
        actor_id=current_user["user_id"], actor_role=current_user["role"],
        notes=f"Livré — preuve : {proof.proof_type}",
        metadata={"proof_type": proof.proof_type, "proof_data": proof.proof_data},
    )
    return updated
